...
First, we create a broad mode decision variable. 

Numbering scheme for the behavioral model:

1. TRANSIT: Bus, Train, Ferry, and School Bus (this is because the majority of students in Australia take public transit to school, so it was assumed that the School Bus variable referred to special routes that TransLink runs for school services)
2. PRIVATEDRIVE: Driving a privately owned vehicle (car or motorcycle). This was made its own alternative because it is only available to those who possess a drivers license (there are observations of people driving and riding in cars they do not own).
3. PRIVATERIDE: Riding in a car or taxi
4. BIKE: Riding a bicycle
5. WALK: Walking
6. PLANE: Traveling by airplane. This is later dropped so that the model can focus only on travel within the region.
7. OTHER: Everything else
Numbering scheme in the trips file:

```{r}
trips <- trips %>% 
  mutate(transit = as.factor(case_when(
    mode %in% c(7,8,9,10) ~ "transit", 
    mode %in% c(1,3) ~ "private drive",
    mode %in% c(2,6) ~ "private ride",
    mode %in% c(5) ~ "bike",
    mode %in% c(4) ~ "walk",
    mode %in% c(6) ~ "plane", 
    mode %in% c(12) ~ "other"
...
1. The only thing that matters is what leg of the trip you are on, the total number of stages in your trip, the number of total trips that you make, the duration of the trip, and your trip distance. 
2. Introduce departure time and trip purpose variables. 
3. Introduce household level variables such as fare, year, weekend, car ownership, bike ownership, and HHIC 
...
In each model, we estimate the probability that an individual chooses a specific mode for a specific leg of their trip, coniditional on the entire set of trips that individual takes. There are five alternatives: 

* Private Transportation
* Public Transportation
* Grouped Rides (Taxi and School Bus)
* Physical Transportation (Walking or Biking)
* Plane 

It is important to note that in our model, we treat an individual activity based decision as dependent on a trip level perspective of transportation choices. 
